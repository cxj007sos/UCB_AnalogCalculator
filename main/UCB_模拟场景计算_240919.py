...
def main():
    while True:
        print("欢迎使用大羽牌UCB多臂老虎机策略_ver240919_模拟场景计算")
        print("——————————————————————————————")

        num_arms = get_valid_num_arms()
        print(f"共有 {num_arms} 个臂，分别对应: {', '.join(string.ascii_uppercase[:num_arms])}")
        ...
        print("\n每个臂的均值:")
        for i, mean in enumerate(arm_means):
            print(f"臂 {string.ascii_uppercase[i]}: {mean}")
        ...
        print("\nUCB模拟设置:")
        num_rounds = int(input(f"请输入要进行UCB模拟的轮数: "))

        arms = initialize_arms(num_arms)
        rewards_history, final_arm = run_ucb_simulation(arms, rewards, num_rounds)
