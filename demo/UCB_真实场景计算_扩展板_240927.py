...
def ucb_value(avg_reward, total_choices, count, rewards, top_two_diff):
    ...
    # 根据最佳两个选项的差距来决定C值
    sigma = 3               # 【标准差的倍数，可根据实际情况调整。sigma = 1 时为UCB1算法偏保守，减少多次其他臂的尝试】
    if top_two_diff > 0.1:  # 【假设0.1为"很大"差距的阈值，需要根据实际情况调整】
        C = sigma * std_dev
    else:
        C = sigma * std_dev * math.log(total_choices)
    
    return avg_reward + C * math.sqrt(2 * math.log(total_choices) / count)
...
def run_ucb_simulation(arms):
    ...
    for round_num in range(1, rounds + 1):
        ...
        ucb_values = {arm: ucb_value(arms[arm]['value'], total_choices, arms[arm]['count'], 
                                     arms[arm]['rewards'], top_two_diff) for arm in arms}
...
def main():
    while True:
        print("欢迎使用大羽牌UCB多臂老虎机策略_扩展板_ver240927")
        print("——————————————————————————————")
